...
**Figures in this folder:** generated by `scripts/07_generate_insight_figures.py`
...
**Training in this repo:** `scripts/train_and_collect_dynamics.py` fine-tunes DistilBERT / RoBERTa-base / RoBERTa-large on SNLI and logs `prob_gold` every epoch. Selection bar charts (`fig03`, `fig04`) still use **published WinoGrande numbers** unless you add full retrain-on-subset runs yourself.
...
| File | Description |
|------|-------------|
| `fig01_data_map_correctness.png` | Main data map: variability (x) vs. confidence (y), points colored by correctness; annotated easy / hard / ambiguous regions |
