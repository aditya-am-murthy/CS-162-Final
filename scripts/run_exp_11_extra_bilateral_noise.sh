...
CMD="cd '$ROOT' && \
  $PYTHON scripts/13_bilateral_noise_flip.py \
    ...
    --reuse-easy-dir '$EASY_REUSE' --no-reuse-easy \
    --gpus '$GPUS' --restarts '$RESTARTS' --no-multi-gpu \
...
exp_print_launch_summary "$SESSION" \
  "GPU: $GPUS (single GPU, no DataParallel) | restarts=$RESTARTS" \
  "subsample: train=$MAX_TRAIN_SAMPLES eval=$MAX_EVAL_SAMPLES batch=$BATCH_SIZE" \
  "flip_ratio: $FLIP_RATIO | epochs: $EPOCHS" \
  "both arms trained locally on subsample (no easy reuse)" \
  "output: $OUTPUT_DIR" \
